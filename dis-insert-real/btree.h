...
#define NVRAM_LEN 1.999 * 1024 * 1024 * 1024 //1G
...
public:
    ...
    static const size_t binsearch_threshold = 64;
};
...
public:
	...
	unsigned long long nventry_count;
	unsigned long long nvbitmap_count;
	unsigned long long nvnode_count;
    BTREE_FRIENDS
...
private:
    ...
    struct inner_node : public node
    {
        ...
        inline inner_node(const unsigned short l)
            : node(l)
        {
			memset(slotkey, 0x0, sizeof(key_type) * innerslotmax);
			memset(childid, 0x0, sizeof(node *) * (innerslotmax + 1));
		}
    ...
    struct leaf_node : public node
    {
        ...
        inline bool isfull() const
        {
			return (leaf_node::count() == leafslotmax);
        }
...
private:
	...
	/// Pointer to the Non-volatile Memory
	VMEM *vmp;
...
public:
    ...
    { 
		vmp = nv_create("/mnt/nvram0", NVRAM_LEN);
		if(vmp == NULL)
    ...
    {
       clear();
	  // cout << "nventry_count "<<nventry_count << " nvbitmap_count " << nvbitmap_count << " nvnode_count " << nvnode_count<< endl;
	   nv_destroy(vmp);
    }
...
private:
	...
	///allocate a leaf node from nvram
	inline leaf_node *nvleaf_alloc(VMEM *vmp)
	{
		leaf_node * n = (leaf_node *)nv_malloc(sizeof(leaf_node), vmp);
		if(n == NULL) {
			cout << " nvleaf alloc failed  " << endl;
		}
		//nvnode_count++;
		return n;
	}
    ...
    inline leaf_node * allocate_leaf()
    {
        ...
        leaf_node* n = new (leaf_node_allocator().allocate(1))
                       leaf_node();
#endif
		leaf_node *n = nvleaf_alloc(vmp);
		memset(n, 0x0, sizeof(leaf_node));
		n->prevleaf = NULL;
		n->nextleaf = NULL;
		n->level = 0; 
		n->slotuse = 0;
...
public:
    ...
    void clear()
    {
	//	cout << "there are " << m_stats.leaves << " leaves and " << m_stats.innernodes << " inner nodes " <<endl;
        if (m_root)
        {
...
private:
    ...
    {
        if (1 && sizeof(n->slotkey) > traits::binsearch_threshold)
        {
    ...
    inline int find_lower_org(const leaf_node* n, const key_type& key) const
    {
    //    if (1 && sizeof(n->slotkey) > traits::binsearch_threshold)
		if(1)
        {
    ...
    inline int leaf_find_lower(const leaf_node* n, const key_type& key) const
    {
		return n->count();
		//return n->get_first_free_bit();
    }

	inline int leaf_find_value(const leaf_node *n, const key_type& key)const
	{
		return 0;
		int leafslotsused = n->count();
...
public:
    ...
    iterator find(const key_type& key)
    {
        ...
        int slot = leaf_find_value(leaf, key);
		if(slot == -1) {
			return end();
		}
#if 1
        return (slot < leaf->count() && key_equal(key, leaf->slotkey[slot]))
               ? iterator(leaf, slot) : end();
#endif
	//	return (slot < leaf->count() && key_equal(key, leaf->slotkey[slot]))? leaf->slotdata[slot] : -1;
    }
...
private:
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
        ...
        {
            leaf_node* leaf = static_cast<leaf_node*>(n);
			unsigned int orig_slotused = leaf->count();
			int slot = leaf_find_lower(leaf, key);

            if (!allow_duplicates && slot < orig_slotused && key_equal(key, leaf->slotkey[slot])) {
            ...
            if (leaf->isfull())
            {
                ...
                // check if insert slot is in the split sibling node
				//orig_slotused = leaf->count();
                //if (slot >= orig_slotused)
                //{
                 //   slot -= orig_slotused;
			...
			BTREE_ASSERT(slot >= 0 && slot <= orig_slotused);

            leaf->slotkey[slot] = key;
            if (!used_as_set) 
				leaf->slotdata[slot] = value;

			nv_flush((char *)(leaf->slotkey+slot), sizeof(key_type));
			if (!used_as_set)
				nv_flush((char *)(leaf->slotdata+slot), sizeof(data_type));

			leaf->bitmap_set(slot);
			nv_flush((char *)leaf->bs, BITS_TO_LONGS(leafslotmax) * sizeof(unsigned long));
			//nvbitmap_count++;

         //   if (splitnode && leaf != *splitnode && slot == orig_slotused - 1)
            if (splitnode && leaf != *splitnode && key > leaf->slotkey[leaf->count() - 1])
            {
    ...
    void split_leaf_node_org(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
		...
		///before split, original leaf should be logged
		leaf_node *shadow_leaf = allocate_leaf();
        std::copy(leaf->slotkey, leaf->slotkey + leaf->slotuse,
                  shadow_leaf->slotkey);
        data_copy(leaf->slotdata, leaf->slotdata + leaf->slotuse,
                  shadow_leaf->slotdata);
        data_copy(leaf->bs, leaf->bs + leaf->slotuse,
                  shadow_leaf->bs);
		nv_flush((char *)shadow_leaf, sizeof(leaf_node));
		nvnode_count++;
		///logging finished
        ...
        data_copy(leaf->slotdata + mid, leaf->slotdata + leaf->slotuse,
                  newleaf->slotdata);
        std::copy(leaf->bs + mid, leaf->bs + leaf->slotuse,
                  newleaf->bs);
#endif
        leaf->slotuse = mid;
        ...
        *_newleaf = newleaf;
		///persistent the new_leaf
		nv_flush((char *)newleaf, sizeof(leaf_node));
		nvnode_count++;
		nv_flush((char *)leaf, sizeof(leaf_node));
		nvnode_count++;
		nv_free(vmp, (char *)shadow_leaf);
    }
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
		...
		unsigned int orig_slotused = leaf->count();
		unsigned int mid = (orig_slotused >> 1);
		std::vector<pair<unsigned long, int> >vec;

		for(int i = 0; i < orig_slotused; i++) {
			vec.push_back(std::make_pair(leaf->slotkey[i], i));
		}
        ...
        leaf_node* newleaf = allocate_leaf();
		unsigned int new_slotused = orig_slotused - mid;
		...
		///before split, original leaf should be logged
		leaf_node *shadow_leaf = allocate_leaf();
        std::copy(leaf->slotkey, leaf->slotkey + orig_slotused,
                  shadow_leaf->slotkey);
        data_copy(leaf->slotdata, leaf->slotdata + orig_slotused,
                  shadow_leaf->slotdata);
		std::copy(leaf->bs, leaf->bs + BITS_TO_LONGS(leafslotmax) * sizeof(unsigned long), shadow_leaf->bs);
		nv_flush((char *)shadow_leaf, sizeof(leaf_node));
		//nvnode_count++;
		///logging finished
		//
		memset(leaf, 0x0, sizeof(leaf_node));
		...
		std::vector<pair<unsigned long, int> >::iterator it;
		int leaf_index = 0, newleaf_index = 0;
		int count = 0;
		for(it = vec.begin(); it != vec.end(); ++it, ++count) {
			if(count < mid) {
				leaf->slotkey[leaf_index] = shadow_leaf->slotkey[it->second];
				leaf->slotdata[leaf_index] = shadow_leaf->slotdata[it->second];
				leaf->bitmap_set(leaf_index);
				leaf_index++;
			}else { 
				newleaf->slotkey[newleaf_index]	= shadow_leaf->slotkey[it->second];
				newleaf->slotdata[newleaf_index] = shadow_leaf->slotdata[it->second];
				newleaf->bitmap_set(newleaf_index);
				newleaf_index++;
			}
		}
		for(int i = mid; i < orig_slotused; i++) {
			leaf->bitmap_clear(i);
		}
        ...
        *_newkey = leaf->slotkey[leaf->count() - 1];
        *_newleaf = newleaf;
		///persistent the new_leaf
		nv_flush((char *)newleaf, sizeof(leaf_node));
		//nvnode_count++;
