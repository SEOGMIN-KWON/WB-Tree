...
#define NVRAM_LEN 4 * 1024 * 1024 * 1024 //1G
			 ...
			 bool _UsedAsSet = false>
				 class btree
				 {
					 ...
					 public:
							 ...
							 { 
								 vmp = nv_create("/mnt/nvram", NVRAM_LEN);
								 if(vmp == NULL)
					 ...
					 private:
								 inline int leaf_find_lower(const leaf_node *n, const key_type& key) const
								 {
									 if (0 && sizeof(n->slotkey) > traits::binsearch_threshold)
									 {
										 ...
										 {
											 int i = 0;
											 while (i < count && key_less(n->slotkey[i], key)) ++i;
					 ...
					 private:
									 ...
									 key_type* splitkey, node** splitnode)
							 {
								 ...
								 {
									 leaf_node* leaf = static_cast<leaf_node*>(n);
									 int slotuse = leaf->count();

									 int slot = leaf_find_lower(leaf, key);

									 if (!allow_duplicates && slot < slotuse && key_equal(key, leaf->slotkey[slot])) {
									 ...
									 std::copy(leaf->slotkey, leaf->slotkey + slotuse, shadow_leaf->slotkey);
									 data_copy(leaf->slotdata, leaf->slotdata + slotuse, shadow_leaf->slotdata);
									 std::copy(leaf->bs, leaf->bs+slotuse, shadow_leaf->bs);
									 //persist the shadow_leaf
									 nv_flush((char *)shadow_leaf, sizeof(leaf_node));

									 // move items and put data item into correct data slot
									 BTREE_ASSERT(slot >= 0 && slot <= slotuse);

									 std::copy_backward(leaf->slotkey + slot, leaf->slotkey + slotuse,
											 leaf->slotkey + slotuse + 1);
									 data_copy_backward(leaf->slotdata + slot, leaf->slotdata + slotuse,
											 leaf->slotdata + slotuse + 1);
									 leaf->slotkey[slot] = key;
									 if (!used_as_set) leaf->slotdata[slot] = value;
									 //leaf->slotuse++;
									 leaf->bitmap_set(slotuse);
							 ...
							 void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
							 {
								 ...
								 int slotuse = leaf->count();
								 unsigned int mid = (slotuse >> 1);

								 BTREE_PRINT("btree::split_leaf_node on " << leaf);

								 leaf_node* newleaf = allocate_leaf();

								 newleaf->slotuse = slotuse - mid;
								 ...
								 leaf_node *shadow_leaf = allocate_leaf();
								 if(!shadow_leaf){
									cout << "error on allocate shadow_leaf " << endl;
								 }
								 std::copy(leaf->slotkey, leaf->slotkey + slotuse,
										 shadow_leaf->slotkey);
								 data_copy(leaf->slotdata, leaf->slotdata + slotuse,
										 shadow_leaf->slotdata);
								 std::copy(leaf->bs, leaf->bs + slotuse, shadow_leaf->bs);
								 nv_flush((char *)shadow_leaf, sizeof(leaf_node));
								 ///logging finished

								 std::copy(leaf->slotkey + mid, leaf->slotkey + slotuse,
										 newleaf->slotkey);
								 data_copy(leaf->slotdata + mid, leaf->slotdata + slotuse,
										 newleaf->slotdata);
								 std::copy(leaf->bs + mid, leaf->bs + slotuse,
										 newleaf->bs);

								 //leaf->slotuse = mid;
								 for(int i = mid; i < slotuse; i++) {
									leaf->bitmap_clear(i);
								 }
