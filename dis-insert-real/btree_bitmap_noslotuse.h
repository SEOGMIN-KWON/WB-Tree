				 ...
				 class btree
				 {
							 ...
							 struct leaf_node : public node
					 {
						 ...
						 inline bool isunderflow() const
						 {
							 ...
							 return (node::slotuse < minleafslots);
#endif
							 return (count() < minleafslots)? true: false;
						 }
									...
									// if (0 && sizeof(n->slotkey) > traits::binsearch_threshold)
										if(0)
									 {
										...
										// if(count == 0) return 0;
										 ...
										 {
											 int i = 0;
											 while (i < slotuse && key_less(n->slotkey[i], key)) ++i;
									 ...
									 else // for nodes <= binsearch_threshold do linear search.
									 {
										 ...
										 while(lo < slotuse && key_less(n->slotkey[lo], key)) ++lo;
										 return lo;
									 }
								 }
									 ...
									 key_type* splitkey, node** splitnode)
							 {
								 ...
								 else // n->isleafnode() == true
								 {
									 ...
									 if (leaf->isfull())
									 {
										 ...
										 // check if insert slot is in the split sibling node
										 orig_slotused = leaf->count(); // update leaf's slotused
									 ...
									 // before moving, copy original contents to the shadow leaf
									 leaf_node *shadow_leaf = allocate_leaf();
									 std::copy(leaf->slotkey, leaf->slotkey + orig_slotused, shadow_leaf->slotkey);
									 data_copy(leaf->slotdata, leaf->slotdata + orig_slotused, shadow_leaf->slotdata);
									 std::copy(leaf->bs, leaf->bs + BITS_TO_LONGS(leafslotmax) * sizeof(unsigned long), shadow_leaf->bs);
									 //persist the shadow_leaf
									 ...
									 std::copy_backward(leaf->slotkey + slot, leaf->slotkey + orig_slotused, leaf->slotkey + orig_slotused + 1);
									 data_copy_backward(leaf->slotdata + slot, leaf->slotdata + orig_slotused, leaf->slotdata + orig_slotused + 1);
									 for(int i = orig_slotused; i > slot; i--){
										leaf->bitmap_set(i);
									 }
...
//								 newleaf->slotuse = leaf->slotuse - mid;
								 ...
								 std::copy(leaf->slotkey + mid, leaf->slotkey + orig_slotused, newleaf->slotkey);
								 data_copy(leaf->slotdata + mid, leaf->slotdata + orig_slotused, newleaf->slotdata);
								 for(int i = 0; i < orig_slotused - mid; i++)
									 newleaf->bitmap_set(i);
