 ...
 * Copyright (C) 2008-2013 Timo Bingmann * * This program is free software: you can redistribute it and/or modify it
...
// *** Settings
static const unsigned int maxslot = 64;
static const unsigned int sigma = 10000000.0;
/// starting number of items to insert
//static const unsigned int minitems = 64;
//static const unsigned int minitems = 1024000 * 40;
//static const unsigned int minitems = 512000 * 6;
//static const unsigned int minitems = 10240 * 2;
static const unsigned int minitems = 5120000 * 5 ;

/// maximum number of items to insert
//static const unsigned int maxitems = 64;
//static const unsigned int maxitems = 1024000 * 40;
//static const unsigned int maxitems = 10240 * 2;
//static const unsigned int maxitems = 512000 * 6;
static const unsigned int maxitems = 5120000 * 5;
...
//		std::minstd_rand0 rand_gen(seed);
		...
		std::normal_distribution<double> normalDist(1100000000.0, sigma);
	//	std::exponential_distribution<double> normalDist(0.000001);
//		std::uniform_real_distribution<double> normalDist(0.0, 2200000000);
//		std::exponential_distribution<double> normalDist(0.3);
        for (unsigned int i = 0; i < items; i++) {
		//	unsigned int r = rand();
			unsigned int r = (unsigned int )normalDist(generator);
		//	cout << r << endl;
...
//		if(map.find(*it).data() != *it){
			if(map.find(*it) != *it){
//				cout << "this is a lookup error " << endl;	
			}
		}
#else
        for (unsigned int i = 0; i < items; i++) {
            unsigned int r = rand() ; 
			//unsigned int r = rand() % 64;
			if(map.find(r).data() != r){
//			if(map.find(r) != r){
				cout << "this is a lookup error " << endl;	
//				return -1;
			}
...
public:
   ...
   // typedef TestClass<std::tr1::unordered_multimap<
    ...
    class BtreeMap
        : public TestClass<
              stx::btree_multimap<unsigned long long , unsigned long long,
...
#if 0
	cout << " BtreeMap " <<endl;
    testrunner_loop<BtreeMap<maxslot> >(os, items);
#endif
#if 1
	cout << "SkiplistMap" << endl;
	testrunner_loop<SkiplistMap<maxslot> >(os, items);
#endif
