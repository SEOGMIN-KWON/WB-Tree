...
static inline void cache_defer(struct bplus_tree *tree, struct bplus_node *node)
{
        ...
        tree->used[i] = 0;
		// print cache dereference
		printf("cache[%d] is dereferenced\n",i);
}
...
static struct bplus_node *node_fetch(struct bplus_tree *tree, off_t offset)
{
		...
		// printf node_fetch
		printf("node_fetch arises: include cache_refer\n");
		printf("fetch offset is %ld\n",offset);
        return node;
}
...
static struct bplus_node *node_seek(struct bplus_tree *tree, off_t offset)
{
        ...
        for (i = 0; i < MIN_CACHE_NUM; i++) {
                if (!tree->used[i]) {
                        ...
                        assert(len == _block_size);
						// print node_seek
						printf("called node_seek : cache[%d] is not used yet\n",i);
                        return (struct bplus_node *) buf;
                }
        }
        assert(0);
...
static inline void node_flush(struct bplus_tree *tree, struct bplus_node *node)
{
        if (node != NULL) {
                ...
                assert(len == _block_size);
				// print node_flush
				printf("called node_flush : pwrite & cache defer\n");
				printf("flush node is %p\n",node);
...
static off_t new_node_append(struct bplus_tree *tree, struct bplus_node *node)
{
        ...
        if (list_empty(&tree->free_blocks)) {
				...
				// print new_node_append
				printf("called new_node_append: list_empty case\n");
        } else {
                ...
                free(block);	
				// print new_node_append
				printf("called new_node_append: firstf_free_block_is_assinged\n");	
        }
		// print new_appended_node
		printf("new_appended_node is %ld\n",node->self);
        return node->self;
}
                	...
                	struct bplus_node *left, struct bplus_node *right)
{
        if (left != NULL) {
                if (right != NULL) {
						...
						// print node_delete 
						printf("node_delete>l,r: right_of_mine_delete\n");
                        node_flush(tree, right);
                } else {
                        left->next = INVALID_OFFSET;
						// print node_delete
						printf("node_delete>l\n");
                }
				// print node_delete
				printf("node_delete: left_of_mine_delete\n");
                node_flush(tree, left);
        } else {
                if (right != NULL) {
                        right->prev = INVALID_OFFSET;
						// print node_delete
						printf("node_delete>r\n");
                        node_flush(tree, right);
                }
        }
		...
		// print deleted blocks allocated for other node
		printf("Add the deleted node into FREE_BLOCK_TAILS\n");
		printf("delete node is %ld\n",block->offset);
                		   ...
                		   int index, struct bplus_node *sub_node)
{
		...
		// print sub_node_update
		printf("sub_node_update(non-leaf node의 pointer update) > index : %d\n",index);
...
{		// print sub_node_flush
		printf("sub_node_flsuh(non-leaf node의 pointer flush) > sub_offset : %ld\n",sub_offset);
...
{
        int ret = -1;
		// print bplus_tree_search
		printf("bplus_tree_search\n");
        ...
        while (node != NULL) {
                ...
                if (is_leaf(node)) {
						...
						// print the non-leaf node
						printf("This is non-leaf node\n");
                }
        }
		// print the key
		print("return key is %d\n",ret);
        return ret;
}
...
{
		// print right_node_add
		printf("rigt_node_add\n");
...
{
		// print parent_node_build
		printf("parent_node_build\n");
        ...
        if (l_ch->parent == INVALID_OFFSET && r_ch->parent == INVALID_OFFSET) {
				...
				printf("l_ch->parent에 non_leaf_insert");
                return non_leaf_insert(tree, node_fetch(tree, l_ch->parent), l_ch, r_ch, key);
        } else {
				printf("r_ch->parent에 non_leaf_insert");
